...
document.addEventListener('DOMContentLoaded', async () => {
  try {
    ...
    const makeLineChart = (canvas, datasets, yMin, yMax, yLabel) =>
      new Chart(canvas.getContext('2d'), {
        type: 'line',
        data: { datasets },
        options: {
          maintainAspectRatio: false,
          plugins: { zeroLinePlugin: {}, legend: { display: false }, tooltip: { enabled: false } },
    ...
    uniqueInts.forEach(intensity => {
      ...
      // pupil
      {
        ...
        const avgDS = mkAvgDataset(avg);
        makeLineChart(c, [...raw, avgDS], -10, 25, 'Pupil size (a.u.)');
      }
      ...
      // locomotor
      {
        ...
        const raw = mkRawDatasets(map);
        const avgDS = mkAvgDataset(avg);
        makeLineChart(c, [...raw, avgDS], -30, 100, 'Locomotor activity (a.u.)');
      }
    ...
    renderScatter('size-peaks-chart', sizePeaks, avgSize, -10, 25, 'Pupil size peak (a.u.)');
    renderScatter('vel-peaks-chart',  velPeaks,  avgVel,  -30, 80, 'Locomotor activity peak (a.u.)');
